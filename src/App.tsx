...
import { AirsLink, AirsLinkTrigger, type AirsLinkAppId, type IntelItem } from './components/AirsLink';
...
export function App() {
  ...
  useEffect(() => {
    ...
    if (state.audioEnabled) audio.cue('travel');
  ...
  useEffect(() => {
    const onVisibility = () => { 
      if (document.hidden) {
        ...
        void audio.resume().catch(() => dispatch({ type: 'AUDIO', enabled: false })); 
      }
    };
