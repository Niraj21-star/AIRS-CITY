      ...
      <div className="content-body">
        {d.id === 'hq' && <>
          ...
          {view === 'origin' && (state.chapter < 4 ? <>
            ...
            <p className="editorial-note">Concept manifesto. Final organizational copy awaits AIRS approval.</p>
        ...
        {d.id === 'arena' && <><nav className="chapter-tabs" aria-label="Arena archives">{['next event', 'archive', 'hall of fame'].map(v => <button key={v} className={view === v || (view === 'origin' && v === 'next event') ? 'active' : ''} onClick={() => selectView(v)}>{v}</button>)}</nav>
        {(view === 'next event' || view === 'origin') ? (
          <>
            <p className="eyebrow">Upcoming Event / VORTEXA 3.0</p>
            <h2 className="section-statement">12 hours<br/>Hackathon.</h2>
            <p className="chapter-description">VORTEXA is a 12-hour hackathon that brings together tech-driven students to solve real-world challenges with creativity, speed, and teamwork.</p>
            <div className="build-slot">
              <span className="build-wireframe" aria-hidden="true"><Icon name="arena" size={65}/></span>
              <div>
                <span className="eyebrow">Expected Footfall - 400+</span>
                <h3>Prize Pool<br/>Upto ₹75000</h3>
                <span className="availability">Team Size: 3-5 Members</span>
              </div>
            </div>
            <dl className="case-format">
              <div><dt>Domains</dt><dd>Artificial Intelligence &amp; Machine Learning<br/>Web &amp; App Development<br/>Sustainable Tech<br/>Open Innovation</dd></div>
              <div><dt>Stats</dt><dd>400+ Participants / 30+ Colleges / 80+ Teams</dd></div>
            </dl>
            <button className="primary-action">Register for Vortexa 3.0<Icon name="arrow"/></button>
          </>
        ) : (
          <EditorialEmpty code={view === 'hall of fame' ? 'ARENA / ACHIEVEMENTS' : 'ARENA / MISSION ARCHIVE'} title={view === 'hall of fame' ? 'Good work deserves to be remembered.' : 'Every gathering leaves a mark.'} text={view === 'hall of fame' ? 'A future home for verified competition results, milestones, and the teams that earned them. No achievements have been published yet.' : 'Workshops, hackathons, and shared discoveries will be documented here once confirmed event records are supplied.'} foot={view === 'hall of fame' ? 'Hall of fame / Awaiting verified results' : 'Event archive / No entries published'}/>
        )}
        </>}
