...
export function CityMap({ state, dispatch, reduced, travel, onReady, airsLinkOpen }: Props) {
  ...
  const apply = () => {
    ...
    if (world.current) {
      const isMobile = size.current.width < 760;
      // On mobile overview, keep markers crisp, compact, and clearly readable
      const markerScale = isMobile && scale < 1 ? Math.min(3.2, 0.78 / scale) : 1 / scale;
      gsap.set(world.current, { x, y, scale, force3D: true, '--marker-scale': markerScale });
    }
  };
  ...
  const overview = (): Camera => {
    ...
    if (width >= 760) {
      // Desktop: preserve existing cinematic overview exactly
      return cameraAt({ x: .5, y: .5 }, width, height, worldWidth, worldHeight, 1);
    }

    // ── Mobile Dynamic Full-City Fit Calculation ───────────────────────────
    // Derived from viewport dimensions, master map aspect ratio, and UI safe bounds
    const mapAspect = cityAsset.width / cityAsset.height; // 2600 / 1700 ≈ 1.5294
    const isPortrait = height >= width;

    // Edge padding for comfortable clearance from phone glass edges
    const padX = isPortrait ? Math.max(10, width * 0.03) : Math.max(16, width * 0.05);
    const availWidth = width - padX * 2;

    // Top HUD clearance (world-header, badge, and editorial breathing space)
    const topClearance = isPortrait ? Math.max(88, height * 0.11) : 46;

    // Bottom controls clearance (mobile-districts and footer)
    const bottomClearance = isPortrait ? Math.max(130, height * 0.16) : 58;
    const availHeight = Math.max(120, height - topClearance - bottomClearance);
    ...
    const x = centerX - 0.5 * worldWidth * fitScale;
    const y = centerY - 0.5 * worldHeight * fitScale;
  ...
  useLayoutEffect(() => {
    const measure = () => {
      ...
      const mapAspect = cityAsset.width / cityAsset.height;
      const worldHeight = Math.max(height * 1.08, (width * 1.08) / mapAspect);
      const worldWidth = worldHeight * mapAspect;
  ...
  const changeZoom = (direction: number) => {
    ...
    const minScale = baseOverview.scale;
    const maxScale = 2.4;
    const newScale = Math.max(minScale, Math.min(maxScale, camera.current.scale + direction * (width < 760 ? 0.2 : 0.4)));
    if (newScale <= minScale + 0.005 && direction < 0) {
      animateCamera(baseOverview, .4);
      return;
    }
    const ratio = newScale / camera.current.scale;
    const targetX = width / 2 - (width / 2 - camera.current.x) * ratio;
    const targetY = height / 2 - (height / 2 - camera.current.y) * ratio;
    animateCamera({ x: targetX, y: targetY, scale: newScale }, .4);
  };

  const focusWaypoint = (id: DistrictId) => {
    dispatch({ type: 'SELECT', id });
    const { width, height, worldWidth, worldHeight } = size.current;
    if (camera.current.scale > 1.2) {
      animateCamera(clampCamera(cameraAt(byId[id].position, width, height, worldWidth, worldHeight, camera.current.scale), width, height, worldWidth, worldHeight));
    }
  };
  ...
  return <section className={`city ${state.level !== 'CITY' ? 'city-away' : ''} ${state.isTransitioning ? 'city-traveling' : ''}`} aria-label="Interactive AIRS City map" aria-hidden={state.level !== 'CITY'} inert={state.level !== 'CITY' || state.isTransitioning || !state.introComplete}>
    <div className="map-viewport" ref={viewport}
      ...
      onPointerMove={e => {
        ...
        const { width, height, worldWidth, worldHeight } = size.current;
        const baseOverview = overview();
        if (width < 760 && camera.current.scale <= baseOverview.scale * 1.05) {
          const damp = 0.3;
          camera.current.x = drag.current.cx + dx * damp;
          camera.current.y = drag.current.cy + dy * damp;
        } else {
      ...
      onPointerUp={() => {
        if (drag.current?.moved && size.current.width < 760 && camera.current.scale <= overview().scale * 1.05) {
          animateCamera(overview(), 0.35);
        }
        drag.current = null;
        startBreathing();
      }}
      ...
      <div className="map-world" ref={world}>
        ...
        <svg className="city-connections" viewBox="0 0 1000 654" preserveAspectRatio="none" aria-hidden="true">
          ...
          {/* Arena ↔ Crew */}
          <line x1="500" y1="419" x2="730" y2="432" stroke="white" strokeOpacity=".06" strokeWidth=".7" strokeDasharray="6 14"/>
          {/* Research ↔ Crew */}
          <line x1="745" y1="281" x2="730" y2="432" stroke="white" strokeOpacity=".055" strokeWidth=".65" strokeDasharray="6 14"/>
          {/* Garage ↔ Arena */}
    ...
    <nav className="mobile-districts" aria-label="Select a district">{districts.map(d => <button key={d.id} aria-label={`Navigate to ${d.name}`} aria-pressed={state.selected === d.id} className={state.selected === d.id ? 'active' : ''} style={{ '--accent': d.accent } as CSSProperties} onClick={() => { focusWaypoint(d.id); travel(d.id); }}><Icon name={d.id}/><span>{d.id === 'research' ? 'Research' : d.id === 'garage' ? 'Garage' : d.id === 'arena' ? 'Arena' : d.id === 'crew' ? 'Crew' : 'HQ'}</span></button>)}</nav>
  </section>;
}
